...
if(isset($_POST['cou'])){
	...
	$c=mysqli_query($con,"SELECT * FROM eskills_fact_likes_logs where given_id='$nid' and given_by='$s' ");
	$m=mysqli_num_rows($c);
	if($m>0){
		mysqli_query($con,"INSERT INTO eskills_fact_likes_logs (type,given_id,given_by,given_ip,status) values ('dislike','$nid','$s','$ip','already')");
		echo "3";
	}else{
	$r=mysqli_query($con,"UPDATE eskills_facts SET dislikes=dislikes+1 WHERE id='$nid'");	
	if($r==true){
		...
		mysqli_query($con,"INSERT INTO eskills_fact_likes_logs (type,given_id,given_by,given_ip,status) values ('dislike','$nid','$s','$ip','success')");
		echo "1";
	}else{
		mysqli_query($con,"INSERT INTO eskills_fact_likes_logs (type,given_id,given_by,given_ip,status) values ('dislike','$nid','$s','$ip','fail')");
		echo "2";
		}
	}
}else{
