...
if(isset($_POST['cou'])){
	...
	$m=mysqli_num_rows($c);
	if($m>0){
		mysqli_query($con,"INSERT INTO eskills_fact_likes_logs (type,given_id,given_by,given_ip,status) values ('like','$nid','$s','$ip','already')");
		echo "3";
	}else{
	$r=mysqli_query($con,"UPDATE eskills_facts SET likes=likes+1 WHERE id='$nid'");	
	if($r==true){
		...
		mysqli_query($con,"INSERT INTO eskills_fact_likes_logs (type,given_id,given_by,given_ip,status) values ('like','$nid','$s','$ip','success')");
		echo "1";
	}else{
		mysqli_query($con,"INSERT INTO eskills_fact_likes_logs (type,given_id,given_by,given_ip,status) values ('like','$nid','$s','$ip','fail')");
		echo "2";
		}
	}
}else{
