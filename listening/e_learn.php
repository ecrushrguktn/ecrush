...
$visits=mysqli_query($con,"SELECT * FROM page_logs where dates='$today' and stuid='$stuid' and page='E-Learn'");
$cm=mysqli_num_rows($visits);
  if($cm<=0){
mysqli_query($con,"INSERT INTO page_logs(ip,enter_time,no_of_times,dates,page,hub,stuid) VALUES('$ip','$dat','1','$today','E-Learn','Listening','$stuid')");
mysqli_query($con,"INSERT INTO page_logs_log(ip,enter_time,no_of_times,dates,page,hub,stuid) VALUES('$ip','$dat','1','$today','E-Learn','Listening','$stuid')");
   }else{
mysqli_query($con,"UPDATE page_logs SET no_of_times=no_of_times+1 where dates='$today' and page='E-Learn' and stuid='$stuid'");
mysqli_query($con,"INSERT INTO page_logs_log(ip,enter_time,no_of_times,dates,page,hub,stuid) VALUES('$ip','$dat','1','$today','E-Learn','Listening','$stuid')");        
    }
?>
<!DOCTYPE html>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="page-title-box">                                    
                                  <h4 class="page-title">English Learning Videos</h4>
...
$dates=date('d-m-Y');
$mk=("SELECT * FROM videos where visibility='1' and category='English Learning Videos' order by id DESC");
...
<?php
  $i=0;
  $today=date('Y-m-d');
  $q=("SELECT * FROM videos where visibility='1' and  category='English Learning Videos' ORDER BY id DESC LIMIT 3");
...
<?php
  $i=0;
  $today=date('Y-m-d');
  $q=("SELECT * FROM videos where visibility='1' and  category='English Learning Videos' ORDER BY id");
  $sq=mysqli_query($con,$q);
  if($i%2==0){ $color='pink'; }else if($i%3==0){ $color='purple'; }else{ $color='primary';}
  while($v=mysqli_fetch_array($sq,MYSQLI_BOTH))
  {
